...
def order_points_by_z(pointcloud, normed_pointcloud):
    new_pointcloud = pointcloud[np.argsort(normed_pointcloud[:, 2])]
    colors = np.interp(normed_pointcloud[:, 2], [np.min(normed_pointcloud[:, 2]), np.max(normed_pointcloud[:, 2])], [0, 255])
    return new_pointcloud, colors